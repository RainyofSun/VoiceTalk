//
//  AppGuideAnimationViewController.swift
//  VoiceTalk
//
...
//    private let iftttCloud = UIImageView(image: UIImage(named: "IFTTTCloud"))
    private let iftttCloud = UIButton.init(type: UIButton.ButtonType.custom);
    ...
    private func configureViews () {
        ...
        iftttCloud.setImage(UIImage(named: "IFTTTCloud"), for: UIControl.State.normal);
        iftttCloud.addTarget(self, action: #selector(switchRootVC), for: .touchUpInside);
    ...
    private func configureIFTTTPresents() {
        ...
        NSLayoutConstraint(item: iftttPresents, attribute: .top, relatedBy: .equal, toItem: contentView, attribute: .top, multiplier: 1, constant: CGFloat(kTopBarSafeHeight)).isActive = true
